...
public:
	...
	typedef typename buffered_exchange<vertex_buffer_record>::buffer_type vertex_buffer_type;
	...
	void finalize() {
		...
		/* Prepare matrix ingress */
		{
			...
			if (standalone) { /* fast pass for standalone */
				...
				for (target_hash_table_type::iterator iter = iht.begin(); iter != iht.end(); iter++){
					...
					if (!p_queue.empty()){
						...
						while (!p_queue.empty()){
							...
							p_queue.pop();
							point_lock.unlock();
							if (flaght[p_point] == 0){ // @wenwen flag hash table
				...
				std::bitset<128> lvec;
				std::map<int, int> arrange;
				std::vector<int> reverse_arrange;
				reverse_arrange.resize(matrixSize);
				std::vector<std::bitset<128> > mat_vec(matrixSize); //the partition should no more than 128
				for(int i = 0; i< matrixSize; i++){
					for(int j = 0; j<matrixSize; j++){
						mat_vec[i].set(myMatrix[i][j]);
					}
				}
				for (target_hash_table_type::iterator iter = iht.begin(); iter != iht.end(); iter++){
					lvec.reset();
					if (dht[iter->first] > threshold){
						for (size_t j = 0; j < iht[iter->first].size(); j++){
							if (vht.find(iht[iter->first][j])!=vht.end()){
								lvec.set(vht[iht[iter->first][j]]);
							}
						}
						for (size_t j = 0; j < oht[iter->first].size(); j++){
							if (vht.find(oht[iter->first][j])!=vht.end()){
								lvec.set(vht[oht[iter->first][j]]);
							}
						}
						unsigned long max_match = matrixSize/2;
						int cur_col = -1;
						for(int i = 0; i<matrixSize; i++){
							unsigned long tmp = (mat_vec[i]&lvec).count();
							if(max_match < tmp){
								max_match = tmp;
								cur_col = i;
							}
						...
						if(cur_col==-1)
							cur_col = iter->first % matrixSize;
						arrange[iter->first] = cur_col;
						reverse_arrange[cur_col]++;
					}
				}
				for(int i =0; i<matrixSize; i++){
					std::cout << "column " << i <<": " <<reverse_arrange[i] << std::endl;
				}
				iht.clear();
				oht.clear();
				int low=0,high=0;
					...
					if (dht[rec.target] <= threshold){
						...
						if (owning_proc != l_procid){
							for (int j = 0; j < ceng; j++){
								...
								std::ofstream fout;
								fout.open(buf,std::ios::out|std::ios::app|std::ios::binary);
								fout << rec.source << "\t" << rec.target << std::endl;
								fout.close();
							}
						...
						if (vht.find(rec.source) != vht.end()&&vht[rec.source]!=-1){
							...
							assert(arrange.find(rec.target) != arrange.end());
							row = arrange[rec.target];
	...
	void modified_base_finalize(size_t nedges) {
		...
		// Setup the map containing all the vertices being negotiated by this machine
		{
			...
			if (matrix_vertex_exchange.size() > 0) {
				...
				// 		}
				// 		if (distributed_matrix_block_ingress::vertex_combine_strategy && lvid < graph.num_local_vertices()) {
				// 			distributed_matrix_block_ingress::vertex_combine_strategy(graph.l_vertex(lvid).data(), rec.vdata);
				// 		}
				// 		else {
...
#ifdef TUNING
				logstream(LOG_INFO) << "base::#vert-msgs=" << matrix_vertex_exchange.size() << std::endl;
				if(l_procid == 0) {
		...
		/*Assign vertex data and allocate vertex (meta)dataspace*/
		{
			...
			foreach(const vid2lvid_pair_type& pair, vid2lvid_buffer) {
				...
				if (standalone)
					...
					if (vht.find(pair.first) != vht.end())
						...
						vrec.owner = pair.first % cnodes + l_procid/cnodes*cnodes;
				}
			}
...
#pragma omp parallel
			...
			if (!flying_vids.empty()) {
				...
				for (typename boost::unordered_map<vertex_id_type, mirror_type>::iterator it = flying_vids.begin();it != flying_vids.end(); ++it) 
				{
